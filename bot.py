...
BOT_TOKEN = "8779088701:AAFeSQv1KL-4hOuU0FeSvLY0bT6zFxgE_R8"
REG_ID = 39058108
        ...
        try:
            ...
            while category is None:
                ...
                if category_text in CATEGORY_EMOJIS:
                    ...
                    await conv.send_message("❌ يرجى إرسال اسم تصنيف صحيح: رخيص، غالي، احتيالي، أو مزيف")
...
async def show_u_bot_channel(event):
    channel_url = syyad_conf.get('bot_channel_url', 'https://t.me/H_H_HU')
    await event.edit(
...
async def show_u_support(event):
    await event.edit(
        ...
        buttons=[
            [
                Button.url('📞 الدعم الفني', 'https://t.me/UUF_Y')
            ],
            [
...
async def show_u_buy_menu(event):
    await event.edit(
        ...
        "🪙 **رخيص:** أرقام بأسعار منخفضة\n"
        "👑 **غالي:** أرقام مميزة بأسعار مرتفعة\n"
        "🎭 **احتيالي:** أرقام قد تكون محفوفة بالمخاطر\n"
        "🏮 **مزيف:** أرقام غير حقيقية (استخدام مؤقت)",
        parse_mode='markdown',
        buttons=[
            [Button.inline("🪙 رخيص", data="buy_category:رخيص")],
            [Button.inline("👑 غالي", data="buy_category:غالي")],
...
async def show_u_nums_by_category(event, category):
    ...
    for phone, details in nums_in_category.items():
        ...
        buttons.append([Button.inline(btn_text, data=f"view_number_details:{phone}")])
    ...
    await event.respond(
        f"{cat_emoji} **الأرقام {category} المتاحة:**",
        buttons=buttons
    )
...
async def show_u_chrg(event):
    ...
    await event.respond(
        ...
        f"كل نقطة تساوي نجمة لشراء النقاط راسل المطور.\n\n"
        f"📩 **للتواصل:**",
        parse_mode='markdown',
        buttons=[
            [
                Button.url('📩 مراسلة المطور', 'https://t.me/JJF_Y'),
                Button.url('🚫 التواصل مع المحظورين', 'https://t.me/k_g_fbot')
            ],
            [
                Button.inline('🔙 العودة للقائمة الرئيسية', 'user_main_menu')
            ]
...
async def hndl_a_set_bot_chan(event):
    async with client.conversation(event.sender_id, timeout=120) as conv:
        curr_chan = syyad_conf.get('bot_channel_url', 'https://t.me/H_H_HU')
        await conv.send_message(
